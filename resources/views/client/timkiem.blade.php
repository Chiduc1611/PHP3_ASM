            ...
            <form action="{{ route('views.search') }}" method="post">
                ...
                <div class="d-flex flex-row border rounded-2 ps-2">
                    <input class="border-0 me-2 w-100 rounded-2 ps-3 input-outline" name="nameArticle" type="search"
                        aria-label="Search" placeholder="Tiêu đề" required value="{{ old('nameArticle', $nameArticle) }}"
                        autocomplete="off">
                ...
                <div class="mt-3 row">
                    <div class="col-3">
                        ...
                        <select name="category" class="form-select">
                            ...
                            @foreach ($categorys as $item)
                                <option value="{{ $item->id }}" @if (old('category', $cateSearch) == $item->id) selected @endif>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-3">
                        ...
                        <select name="time" class="form-select">
                            ...
                            <option value="year"@if (old('time', $time) == 'year') selected @endif>1 năm trước</option>
                        </select>
                    </div>
